...
public class Busqueda extends JFrame {
		...
		btnEliminar.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				...
				if (filaReservas >= 0) {
					...
					int confirmarHuesped = JOptionPane.showConfirmDialog(null, "¿Está seguro de borrar al huesped?");
		...
		Optional.ofNullable(modelo.getValueAt(tbReservas.getSelectedRow(), tbReservas.getSelectedColumn()))
				.ifPresent(fila -> {
					...
					try {
						...
						dateEntrada = LocalDate.parse(modelo.getValueAt(tbReservas.getSelectedRow(), 1).toString(),
								dateFormat);
						dateSalida = LocalDate.parse(modelo.getValueAt(tbReservas.getSelectedRow(), 2).toString(),
								dateFormat);
					} catch (DateTimeException e) {
						throw new RuntimeException(e);
					}
					this.reservaVista.limpiarValor();
					String valor = "Bs. " + calcularValorReserva(dateEntrada, dateSalida);
					...
					if (tbReservas.getSelectedColumn() == 0) {
						...
						this.reservasControl.actualizarReserva(dateEntrada, dateSalida, valor, formaPago, id);
	...
	private List<Huespedes> MostrarHuespedes() {
		return this.huespedesControl.mostrarHuespedes();
	}
		...
		Optional.ofNullable(modeloHuesped.getValueAt(tbHuespedes.getSelectedRow(), tbHuespedes.getSelectedColumn()))
				.ifPresentOrElse(filaHuespedes -> {
					...
					LocalDate fechaNacimiento = LocalDate
							.parse(modeloHuesped.getValueAt(tbHuespedes.getSelectedRow(), 3).toString(), dateFormat);
					...
					if (tbHuespedes.getSelectedColumn() == 0 || tbHuespedes.getSelectedColumn() == 6) {
						...
						this.huespedesControl.actualizarHuesped(nombre, apellido, fechaNacimiento, nacionalidad,
								telefono, id_reserva, id);
						JOptionPane.showMessageDialog(this, String.format("Registro modificado correctamente"));
					}

				}, () -> JOptionPane.showInternalMessageDialog(this, "Registro incorrecto"));
	}
